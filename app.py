...
from flask import redirect, render_template, request, session, flash, abort, url_for, g
...
def new_restaurants():
    ...
    if request.method == "GET":
        # Restore saved form data if redirected back from "add tag"
    ...
    if action == "add_tag":
        ...
        # ✅ Save current form data so it’s restored after redirect
    ...
    # --- If user clicked "add restaurant" ---
    elif action == "add_restaurant":
        restaurants.add_restaurant(name, address, link, user_id)
        restaurant_id = restaurants.get_restaurant_by_name(name)[0]

        if selected_tags:
            for tag_id in selected_tags:
                tags.associate_tag_with_restaurant(restaurant_id, tag_id)
